...
class DiegoWTsStarterSelection
  ...
  def pbChooseBall
    ...
    if @pokemon.types[1] != @pokemon.types[0] && StarterSelSettings::TYPE2COLOR && @pokemon.types[1]
      @sprites["textbox"].text = _INTL("<ac>Então, você escolhe #{@pkmnname}, <br>dos tipos #{type1}/#{type2}?</ac>")
    elsif StarterSelSettings::TYPE2COLOR
      @sprites["textbox"].text = _INTL("<ac>Então, você escolhe #{@pkmnname}, <br>do tipo #{type1}?</ac>")
    else
      @sprites["textbox"].text = _INTL("<ac>Então, você escolhe #{@pkmnname}, <br>do tipo #{type1}?</ac>")
    end
    ...
    if confirm == 1
      ...
      @sprites["textbox"].text = _INTL("<ac>Escolha o seu Pokémon.</ac>")
      10.times do
        pbUpdateSpriteHash(@sprites)
        @sprites["pkmn_#{@select}"].opacity -= 255/10
        pbWait(0.012)
